...
class CodegenZENDNN : public MemoizedExprTranslator<std::vector<Output>>, public CodegenCBase {
 public:
  explicit CodegenZENDNN(const std::string& id) { this->ext_func_id_ = id; }

  std::vector<Output> VisitExprDefault_(const Object* op) final {
    LOG(FATAL) << "ZENDNN codegen doesn't support: " << op->GetTypeKey();
  }
 ...
 private:
  ...
  int const_idx_{0};
  /*! \brief The arguments used by a wrapped function that calls ZENDNN kernels. */
  Array<Var> ext_func_args_;
  /*! \brief Statement of the function that will be compiled using ZENDNN kernels. */
  ...
  friend class ZENDNNModuleCodegen;
};

/*!
 * \brief The ZENDNN codegen helper to generate wrapepr function calls of ZENDNN
 * libraries. The code is a CSourceModule that can be compiled separately and
 * linked together with a DSOModule.
 */
class ZENDNNModuleCodegen : public CSourceModuleCodegenBase {
 public:
  // Create a corresponding ZENDNN function for the given relay Function.
  std::pair<std::string, Array<String>> GenZENDNNFunc(const Function& func) {
    ICHECK(func.defined()) << "Input error: expect a Relay function.";

    // Record the external symbol for runtime lookup.
    auto sid = GetExtSymbol(func);

    CodegenZENDNN builder(sid);
...
/*! \brief Serializer to ZENDNN JSON runtime module */
class ZENDNNJSONSerializer : public backend::contrib::JSONSerializer {
...
runtime::Module ZENDNNCompiler(const ObjectRef& ref) {
  ...
  const auto* pf = runtime::Registry::Get("runtime.ZENDNNJSONRuntimeCreate");
...
TVM_REGISTER_GLOBAL("relay.ext.zendnn").set_body_typed(ZENDNNCompiler);

/*!
 * \brief Constant Updater for ZENDNN JSON runtime
 *
 ...
 * the graph as ZENDNNJSONSerializer.
 */
struct ZENDNNConstantUpdater : public ConstantUpdater {
 public:
 ...
 */
Map<String, runtime::NDArray> ZENDNNConstantUpdaterFunc(Expr expr, std::string symbol) {
  // Visit all suitable constant nodes
  std::unordered_map<std::string, runtime::NDArray> res;
  ZENDNNConstantUpdater const_updater(symbol, &res);
  const_updater(expr);
