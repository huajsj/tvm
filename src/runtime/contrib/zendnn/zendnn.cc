...
inline zendnn::memory::desc GenZENDNNMemDescByShape(const zendnn::memory::dims& shape,
  ...
  zendnn::memory::desc data_md;

  switch (shape.size()) {
    case 2:
      data_md = zendnn::memory::desc({shape, dtype, tag::ab});
      break;
    case 3:
      data_md = zendnn::memory::desc({shape, dtype, tag::abc});
      break;
    case 4:
      data_md = zendnn::memory::desc({shape, dtype, tag::abcd});
      break;
    case 5:
...
extern "C" void zendnn_conv2d(float* data, float* weights, float* out, int p_N_, int p_C_, int p_H_,
  ...
  return zendnn_conv2d_common(data, weights, bias.data(), out, p_N_, p_C_, p_H_, p_W_, p_O_, p_G_,
...
extern "C" void zendnn_fused_conv2d_relu(float* data, float* weights, float* out, int p_N_, int p_C_,
...
extern "C" void zendnn_fused_conv2d_bias_relu(float* data, float* weights, float* bias, float* out,
                                            ...
                                            int p_Sw_) {
  return zendnn_conv2d_common(data, weights, bias, out, p_N_, p_C_, p_H_, p_W_, p_O_, p_G_, p_Ph0_,
...
extern "C" void zendnn_bn(float* data, float* gamma, float* beta, float* mean, float* variance,
...
TVM_REGISTER_GLOBAL("tvm.contrib.zendnn.conv2d").set_body([](TVMArgs args, TVMRetValue* ret) {
  ...
  std::vector<float> bias(p_O_, 0);
  primitive_attr attr;
  return zendnn_conv2d_common(static_cast<float*>(input->data), static_cast<float*>(weights->data),
