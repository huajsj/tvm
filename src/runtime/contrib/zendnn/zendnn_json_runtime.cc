...
/*!
 * \file src/runtime/contrib/zendnn/zendnn_json_runtime.cc
...
// TODO(@apeskov): Have to mute warning from zendnn headers.
//  -Wzero-as-null-pointer-constant and -Wdocumentation-unknown-command
//#include <zendnn.hpp>

//#include "zendnn_tensor_requisite.h"
//#include "zendnn_utils.h"
...
class ZENDNNJSONRuntime : public JSONRuntimeBase {
 public:
  ZENDNNJSONRuntime(const std::string& symbol_name, const std::string& graph_json,
  ...
  const char* type_key() const override { return "zendnn_json"; }
  ...
  void Run(const TVMArgs& args) const {
    ...
    for (const auto& act : net_) {
      ...
      // Find proper zendnn::memory buffers
      std::unordered_map<int, zendnn::memory> mem_args;
      ...
      if (prim.get_kind() == zendnn::primitive::kind::reorder) {
        const auto& mem_src = mem_args.at(ZENDNN_ARG_SRC);
        const auto& mem_dst = mem_args.at(ZENDNN_ARG_DST);
 ...
 private:
	/*
  const std::map<std::string, zendnn::algorithm> elt_name2algo{
      ...
      {"logsumexp", zendnn::algorithm::eltwise_logsigmoid},
      {"nn.relu", zendnn::algorithm::eltwise_relu},
      {"nn.leaky_relu", zendnn::algorithm::eltwise_relu},
      {"tanh", zendnn::algorithm::eltwise_tanh},
      {"sigmoid", zendnn::algorithm::eltwise_logistic},
      {"clip", zendnn::algorithm::eltwise_clip},
      {"gelu_erf", zendnn::algorithm::eltwise_gelu_erf},
  };

  zendnn::primitive_attr ParseAttrs(const size_t& nid, TensorRequisite* bias_tr) {
    ...
    if (dst_zp_tr) {
      ...
      auto ops = attr.get_post_ops();
      ops.append_eltwise(1.0, zendnn::algorithm::eltwise_linear, 1.0, zp);
      attr.set_post_ops(ops);
    }
    *bias_tr = GetInputByName(nid, "bias_idx");
    ...
    if (std::regex_match(op_name, relu_pat)) {
      ops.append_eltwise(1.f, zendnn::algorithm::eltwise_relu, 0.f, 0.f);
    }
    if (std::regex_match(op_name, tanh_pat)) {
      ops.append_eltwise(1.f, zendnn::algorithm::eltwise_tanh, 0.f, 0.f);
    }
    if (std::regex_match(op_name, clip_pat)) {
      ...
      ops.append_eltwise(1.f, zendnn::algorithm::eltwise_clip, a_min, a_max);
    }
    if (std::regex_match(op_name, sigmoid_pat)) {
      ops.append_eltwise(1.f, zendnn::algorithm::eltwise_logistic, 0.f, 0.f);
    }
    if (std::regex_match(op_name, swish_pat)) {
      ops.append_eltwise(1.f, zendnn::algorithm::eltwise_swish, 1.f, 1.f);
    }
    if (std::regex_match(op_name, gelu_pat)) {
      ops.append_eltwise(1.f, zendnn::algorithm::eltwise_gelu_erf, 0.f, 0.f);
    }
    if (std::regex_match(op_name, mish_pat)) {
      ops.append_eltwise(1.f, zendnn::algorithm::eltwise_mish, 1.f, 0.f);
    }
    if (ops.len() != 0) {
      attr.set_post_ops(ops);
    }
  ...
  void BuildEngine() {
		/*
    engine_ = zendnn::engine(zendnn::engine::kind::cpu, 0);
    stream_ = zendnn::stream(engine_);
    ...
    for (size_t nid = 0; nid < nodes_.size(); ++nid) {
      ...
      if (node.GetOpType() == "kernel") {
        ...
        if (std::regex_match(op_name, deconv_pat) ||
          ...
          Pooling(nid, zendnn::algorithm::pooling_max);
        } else if (std::regex_match(op_name, avg_pool_pat)) {
          Pooling(nid, zendnn::algorithm::pooling_avg);
        } else if (elt_name2algo.count(op_name)) {
          Eltwise(nid);
        } else if ("nn.softmax" == op_name) {
          Softmax(nid);
        } else if ("add" == op_name) {
          Binary(nid, zendnn::algorithm::binary_add);
        } else if ("multiply" == op_name) {
          Binary(nid, zendnn::algorithm::binary_mul);
  ...
  void Convolution(const size_t& nid) {
    ...
    attr.set_scratchpad_mode(zendnn::scratchpad_mode::user);
    ...
    // Minus one for ZENDNN representation. No dilation for ZENDNN is 0, for relay is 1.
    ...
    if (wgh_tr.dims()[0] != groups || wgh_tr.dims()[1] != dst_tr.dims()[1] / groups ||
      ...
      auto zero_offset = zendnn::memory::dims(wgh_tr.dims().size(), 0);
      wgh_tr = wgh_tr.Crop(wgh_croped_dims, zero_offset);
    }
    // Conv description.
    auto conv_desc = zendnn::convolution_forward::desc(
        zendnn::prop_kind::forward_inference, zendnn::algorithm::convolution_direct,
    ...
    auto conv_prim_desc = zendnn::convolution_forward::primitive_desc(conv_desc, attr, engine_);
    ...
    Submit(zendnn::convolution_forward(conv_prim_desc),
           {{ZENDNN_ARG_SRC, src_tr},
            {ZENDNN_ARG_WEIGHTS, wgh_tr},
            {ZENDNN_ARG_BIAS, bias_tr},
            {ZENDNN_ARG_SCRATCHPAD, scratchpad_tr},
            {ZENDNN_ARG_DST, dst_tr}},
           {sum_in_tr, ZENDNN_ARG_DST});
		*/
  }
  ...
  void Deconvolution(const size_t& nid) {
    ...
    bias_tr = bias_tr.Reshape({dst_tr.dims()[1]});
    // Conv description.
    auto deconv_desc = zendnn::deconvolution_forward::desc(
        zendnn::prop_kind::forward_inference, zendnn::algorithm::deconvolution_direct,
        src_tr.LayoutAny().desc(), wgh_tr.LayoutAny().desc(), bias_tr.LayoutAny().desc(),
        dst_tr.LayoutAny().desc(), strides, dilates, padding_l, padding_r);

    // Enable elementwise post-ops.
    auto deconv_prim_desc = zendnn::deconvolution_forward::primitive_desc(deconv_desc, attr, engine_);
    ...
    Submit(zendnn::deconvolution_forward(deconv_prim_desc), {{ZENDNN_ARG_SRC, src_tr},
                                                           {ZENDNN_ARG_WEIGHTS, wgh_tr},
                                                           {ZENDNN_ARG_BIAS, bias_tr},
                                                           {ZENDNN_ARG_SCRATCHPAD, scratchpad_tr},
                                                           {ZENDNN_ARG_DST, dst_tr}});
  ...
  void Dense(const size_t& nid) {
    ...
    bias_tr = bias_tr.Reshape({dst_tr.dims()[1]});
    // Dense description.
    auto dense_desc = zendnn::inner_product_forward::desc(
        zendnn::prop_kind::forward_inference, src_tr.LayoutAny().desc(), wgh_tr.LayoutAny().desc(),
        bias_tr.LayoutAny().desc(), dst_tr.LayoutAny().desc());

    // Enable elementwise post-ops.
    auto dense_prim_desc = zendnn::inner_product_forward::primitive_desc(dense_desc, attr, engine_);
    ...
    Submit(zendnn::inner_product_forward(dense_prim_desc),
           {{ZENDNN_ARG_SRC, src_tr},
            {ZENDNN_ARG_WEIGHTS, wgh_tr},
            {ZENDNN_ARG_BIAS, bias_tr},
            {ZENDNN_ARG_SCRATCHPAD, scratchpad_tr},
  ...
  void BatchMatMul(const size_t& nid) {
    ...
    // Matmul description.
    auto bmm_desc = zendnn::matmul::desc(src_tr.LayoutAny().desc(), wgh_tr.LayoutAny().desc(),
                                       bias_tr.LayoutAny().desc(), dst_tr.LayoutAny().desc());

    // Enable elementwise post-ops.
    auto bmm_prim_desc = zendnn::matmul::primitive_desc(bmm_desc, attr, engine_);
    ...
    Submit(zendnn::matmul(bmm_prim_desc), {{ZENDNN_ARG_SRC, src_tr},
                                         {ZENDNN_ARG_WEIGHTS, wgh_tr},
                                         {ZENDNN_ARG_BIAS, bias_tr},
                                         {ZENDNN_ARG_SCRATCHPAD, scratchpad_tr},
                                         {ZENDNN_ARG_DST, dst_tr}});
  ...
  void BatchNorm(const size_t& nid) {
    ...
    auto bn_desc = zendnn::batch_normalization_forward::desc(
        zendnn::prop_kind::forward_inference, src_tr.desc(), epsilon,
        zendnn::normalization_flags::use_global_stats | zendnn::normalization_flags::use_scale_shift);
    auto bn_prim_desc = zendnn::batch_normalization_forward::primitive_desc(bn_desc, engine_);
    ...
    auto register_copy = [this](const TensorRequisite& src, const TensorRequisite& dst) {
      zendnn::reorder::primitive_desc copy_pd(engine_, src.desc(), engine_, dst.desc());
      Submit(zendnn::reorder(copy_pd), {{ZENDNN_ARG_SRC, src}, {ZENDNN_ARG_DST, dst}});
    };

    register_copy(gamma_tr, scale_tr);
    register_copy(beta_tr, shift_tr);

    Submit(zendnn::batch_normalization_forward(bn_prim_desc), {{ZENDNN_ARG_SRC, src_tr},
                                                             {ZENDNN_ARG_DST, dst_tr},
                                                             {ZENDNN_ARG_SCALE_SHIFT, scale_shift_tr},
                                                             {ZENDNN_ARG_MEAN, mean_tr},
                                                             {ZENDNN_ARG_VARIANCE, var_tr}});
		*/
  }
  ...
  void LayerNorm(const size_t& nid) {
    ...
    // LN description.
    auto lnorm_desc = zendnn::layer_normalization_forward::desc(
        zendnn::prop_kind::forward_inference, src_tr.desc(), epsilon,
        zendnn::normalization_flags::use_scale_shift);

    auto lnorm_prim_desc = zendnn::layer_normalization_forward::primitive_desc(lnorm_desc, engine_);
    ...
    Submit(
        zendnn::layer_normalization_forward(lnorm_prim_desc),
        {{ZENDNN_ARG_SRC, src_tr}, {ZENDNN_ARG_DST, dst_tr}, {ZENDNN_ARG_SCALE_SHIFT, scale_shift_tr}});
		*/
  }
	/*
  void Pooling(const size_t& nid, zendnn::algorithm algo) {
    auto node = nodes_[nid];
    auto op_name = node.GetOpName();
    ...
    if (!is_global && algo == zendnn::algorithm::pooling_avg) {
      auto include_pad = GetNodeAttr<bool>(node, "count_include_pad");
      algo = include_pad ? zendnn::algorithm::pooling_avg_include_padding
                         : zendnn::algorithm::pooling_avg_exclude_padding;
    }

    // Pooling description.
    auto pool_desc = zendnn::pooling_v2_forward::desc(
        zendnn::prop_kind::forward_inference, algo, src_tr.desc(),  //<= Do not use any for src tensor
        dst_tr.LayoutAny().desc(), strides, kernel, dilates, padding_l, padding_r);
    auto pool_prim_desc = zendnn::pooling_v2_forward::primitive_desc(pool_desc, engine_);
    ...
    Submit(zendnn::pooling_v2_forward(pool_prim_desc),
           {{ZENDNN_ARG_SRC, src_tr}, {ZENDNN_ARG_DST, dst_tr}, {ZENDNN_ARG_SCRATCHPAD, scratchpad_tr}});
...
*/
  ...
  void Eltwise(const size_t& nid) {
    ...
    auto elt_desc = zendnn::eltwise_forward::desc(zendnn::prop_kind::forward_inference, algo,
                                                src_tr.desc(), alpha, beta);
    auto elt_prim_desc = zendnn::eltwise_forward::primitive_desc(elt_desc, engine_);
    ICHECK(src_tr.desc() == elt_prim_desc.dst_desc());

    Submit(zendnn::eltwise_forward(elt_prim_desc), {{ZENDNN_ARG_SRC, src_tr}, {ZENDNN_ARG_DST, dst_tr}});
		*/
  }
  ...
  void Softmax(const size_t& nid) {
    ...
    auto softmax_desc =
        zendnn::softmax_forward::desc(zendnn::prop_kind::forward_inference, src_tr.desc(), axis);
    auto softmax_prim_desc = zendnn::softmax_forward::primitive_desc(softmax_desc, engine_);
    ICHECK(dst_tr.desc() == softmax_prim_desc.dst_desc());

    Submit(zendnn::softmax_forward(softmax_prim_desc),
           {{ZENDNN_ARG_SRC, src_tr}, {ZENDNN_ARG_DST, dst_tr}});
		*/
  }
	/*
  void Binary(const size_t& nid, zendnn::algorithm algo) {
    auto node = nodes_[nid];
    ...
    auto binary_desc = zendnn::binary::desc(algo, lhs_tr.desc(), rhs_tr.desc(), dst_tr.desc());
    auto binary_prim_desc = zendnn::binary::primitive_desc(binary_desc, engine_);

    Submit(zendnn::binary(binary_prim_desc),
           {{ZENDNN_ARG_SRC_0, lhs_tr}, {ZENDNN_ARG_SRC_1, rhs_tr}, {ZENDNN_ARG_DST, dst_tr}});
...
*/
  ...
  TensorRequisite GetInput(const size_t& nid, const int idx) {
    ...
    if (const_dl_tensor) {
      ...
      auto mem = zendnn::memory(desc, engine_, const_dl_tensor->data);
  ...
  void Submit(const zendnn::primitive& prim, const std::unordered_map<int, TensorRequisite>& tr_args,
    ...
    if (auto tr = inplace_conf.first) {
      ...
      zendnn::reorder::primitive_desc io_copy_pd(engine_, tr.desc(), engine_, dst_tr.desc());
      net_.push_back(
          {zendnn::reorder(io_copy_pd), {{ZENDNN_ARG_SRC, arg_id}, {ZENDNN_ARG_DST, dst_arg_id}}});
    }
  ...
  /* The zendnn engine. */
  //zendnn::engine engine_;
  /* The zendnn stream. */
  //zendnn::stream stream_;
  /* The network layers that are represented in zendnn primitives. */
...
runtime::Module ZENDNNJSONRuntimeCreate(String symbol_name, String graph_json,
                                      const Array<String>& const_names) {
  auto n = make_object<ZENDNNJSONRuntime>(symbol_name, graph_json, const_names);
  return runtime::Module(n);
}

TVM_REGISTER_GLOBAL("runtime.ZENDNNJSONRuntimeCreate").set_body_typed(ZENDNNJSONRuntimeCreate);

TVM_REGISTER_GLOBAL("runtime.module.loadbinary_zendnn_json")
    .set_body_typed(JSONRuntimeBase::LoadFromBinary<ZENDNNJSONRuntime>);
