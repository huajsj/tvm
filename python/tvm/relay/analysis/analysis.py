...
def PipelineGraph(expr, indexs):
    ...
    def _recursion(anf, operator_indx, pipeLineMods, indexs):
        if isinstance(anf, tvm.relay.Function):
            return tvm.relay.Function(anf.params,
                                  _recursion(anf.body, operator_indx, pipeLineMods, indexs),
        ...
        if isinstance(anf, tvm.relay.expr.Let):
            ...
            if isinstance(value, tvm.relay.expr.Call):
                if isinstance(value.op, tvm.ir.Op):
                    if (len(indexs) and operator_indx == indexs[0]):
                        indexs.pop(0)
                        ann = _recursion(anf.body, operator_indx, pipeLineMods, indexs)
            ...
            return tvm.relay.expr.Let(anf.var,
				  value,
				  _recursion(anf.body, operator_indx, pipeLineMods, indexs))
        else:
            return anf
    ...
    ann = _recursion(anf, operator_indx, pipeLineMods, indexs)
